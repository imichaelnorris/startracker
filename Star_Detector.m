...
%                   ox and oy (focal lengths and principal point)                   
...
thresh = mean(im_vis_blur(:)) + 7 * std(im_vis_blur(:));
...
while k <= size(approx_centers,1)
    ...
    i_range = max(i_c-win_rad,0):min(i_c+win_rad,size(im_vis,1));
    j_range = max(j_c-win_rad,0):min(j_c+win_rad,size(im_vis,2));
%    i_range = i_c-win_rad:i_c+win_rad;
%    j_range = j_c-win_rad:j_c+win_rad;
    [Jr,Ir] = meshgrid(j_range,i_range);
    try
        A = Ir .* im_vis_blur(i_range, j_range);
        first_mom_i = sum(A(:)); % first moment in i direction
        A = Jr .* im_vis_blur(i_range, j_range);
        first_mom_j = sum(A(:)); % first moment in j direction
        A = im_vis_blur(i_range, j_range);
        TB = sum( A(:) ); %total vis-spectrum brightness
        A = im_uv_blur(i_range, j_range);
...
body_vectors = [ (centers(:,1)-ox)/fx, (centers(:,2)-oy)/fy, ones(size(approx_centers,1),1) ];



%%%%%%% FIX THIS -> num_stars may not match num centers at this point
...
if showIm
    figure;
    imshow(im_bin);
    title('binary thresholded visual spectrum image')
    figure;
    imshow(im_vis);
    hold on
    ...
    % If image was generated with all stars, plot again, highlighting only
    % the ground truth for bright stars
    if nargin > 4 && fullDB
        figure;
        imshow(im_vis);
        hold on
        ...
        for k = 1:size(im_list_bright,1)
            if im_list_b_class(k) < 5
                plot(im_list_bright(k,1),im_list_bright(k,2),'.m');
            else
                plot(im_list_bright(k,1),im_list_bright(k,2),'.y');
            end
        end
        title('o = Detected Stars, . = Ground Truth (optional), Magenta = UV-active, Yellow = Not UV-active')
    end
